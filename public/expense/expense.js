...
async function expense(e) {
    ...
    await axios.post("http://54.144.20.123:8080/add-expense", expenseDetails, { headers: { "Authorization": token } })
        .then(res => {
...
async function deleteExpense(id) {
    await axios.delete(`http://54.144.20.123:8080/delete-expense/${id}`, { headers: { "Authorization": token } })
        .then((res) => {
...
document.getElementById('rzp-btn').onclick = async function (e) {
    const response = await axios.get('http://54.144.20.123:8080/purchase/premiummembership', { headers: { "Authorization": token } });

    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id, // for one time payment
        "handler": async function (response) {
            const res = await axios.post('http://54.144.20.123:8080/purchase/updatetransactionstatus', {
...
async function leaderBoard() {
    const userLeaderBoardArray = await axios.get('http://54.144.20.123:8080/premium/showLeaderBoard',
        { headers: { "Authorization": token } })
...
function showDownloadBtn() {
    ...
    document.getElementById('downloadReport').onclick = function (e) {
        axios.get('http://54.144.20.123:8080/premium/user/download', { headers: { 'Authorization': token } })
            .then((response) => {
...
async function changePage(page, limit) {
    ...
    await axios.get(`http://54.144.20.123:8080/premium/user/reports?page=${page}&limit=${limit}`,
        { headers: { 'Authorization': token } })
        .then((response) => {
